...
public class DemoGlideHelper {
    ...
    private static final TransitionFactory<Drawable> TRANSITION_FACTORY = new TransitionFactory<Drawable>() {
        @Override
        public Transition<Drawable> build(DataSource dataSource, boolean isFirstResource) {
                // Do not animate if image is loaded from memory
                return ((dataSource == DataSource.REMOTE) ? TRANSITION : new NoTransition<Drawable>());
        }
    };
    ...
    public static void loadFlickrThumb(Photo photo, ImageView image) {
        ...
        Glide.with(image).load(photo.getUri())
                .apply(options)
                .thumbnail(thumbRequest)
                .into(image);
    }

    public static void loadFull(Photo photo, ImageView image, LoadingListener listener) {
        final RequestOptions options = new RequestOptions()
                .diskCacheStrategy(DiskCacheStrategy.DATA)
                .override(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL)
                .dontTransform();
